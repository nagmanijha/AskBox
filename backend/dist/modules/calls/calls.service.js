...
class CallsService {
    ...
    async getCalls(page = 1, pageSize = 20, filters = {}) {
        try {
            ...
            const countCheck = await connection_1.pool.query('SELECT COUNT(*) FROM call_telemetry');
            ...
            const result = await connection_1.pool.query(query, queryParams);
            ...
            logger_1.logger.error('Failed to query PostgreSQL for call logs', { error });
            return this.getMockCalls(page, pageSize);
        }
    }
    ...
    async getCallById(id) {
        try {
            const result = await connection_1.pool.query(`SELECT event_type, timestamp, data 
                 FROM call_telemetry 
                 WHERE call_id = $1 
            ...
            logger_1.logger.error('Failed to get call from PostgreSQL', { error, id });
            return null;
        }
    }
