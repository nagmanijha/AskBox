...
export default function LandingPage() {
    ...
    return (
        <div className={`font-display min-h-screen transition-colors duration-300 ${
            isModern ? 'bg-indigo-50 text-slate-900' : 'bg-background-dark text-slate-100'
        }`}>

            {/* Top Navigation */}
            <nav className={`sticky top-0 z-50 w-full backdrop-blur-md px-6 lg:px-20 py-4 ${
                isModern ? 'bg-white/80 border-b border-indigo-100' : 'bg-background-dark/80 border-b border-primary/10'
            }`}>
                <div className="max-w-7xl mx-auto flex items-center justify-between">
                    <div className="flex items-center gap-2">
                        <span className={`material-symbols-outlined text-3xl ${isModern ? 'text-indigo-600' : 'text-primary'}`}>record_voice_over</span>
                        <span className={`text-xl font-bold tracking-tight uppercase tracking-widest ${isModern ? 'text-slate-900' : 'text-slate-100'}`}>AskBox</span>
                    </div>
                    <div className="hidden md:flex items-center gap-10">
                        <div className="flex gap-8">
                            {['Our Vision', 'Impact', 'Technology'].map(item => (
                                <a key={item} href="#" className={`text-sm font-medium transition-colors ${
                                    isModern ? 'text-slate-600 hover:text-indigo-600' : 'text-slate-300 hover:text-primary'
                                }`}>{item}</a>
                            ))}
                        </div>
                        <button 
                             ...
                             className={`p-2 rounded-full transition-colors ${
                                 isModern ? 'bg-slate-100 text-indigo-600 hover:bg-indigo-50' : 'bg-white/5 text-primary hover:bg-white/10'
                             }`}
                        >
                        ...
                        <Link to="/login" className={`px-6 py-2 rounded-lg font-bold text-sm hover:brightness-110 transition-all ${
                            isModern ? 'bg-indigo-600 text-white shadow-md' : 'bg-primary text-background-dark'
                        }`}>
                            Sign In
                        </Link>
                    </div>
            ...
            <section className={`relative min-h-[90vh] flex flex-col justify-center px-6 lg:px-20 overflow-hidden ${
                ...
                <div className="absolute inset-0 z-0 flex items-center justify-center opacity-30 pointer-events-none">
                    <div className="w-full h-[400px] flex items-end justify-center gap-1">
                        ...
                        {[...Array(13)].map((_, i) => (
                            <div key={i} className={`w-2 rounded-full ${
                                i % 2 === 0 
                                ? (isModern ? 'bg-indigo-400' : 'bg-primary') 
                                : (isModern ? 'bg-teal-400' : 'bg-teal-500')
                ...
                <div className="relative z-10 max-w-4xl">
                    <span className={`inline-block px-4 py-1 rounded-full text-xs font-bold uppercase tracking-widest mb-6 ${
                        isModern ? 'bg-indigo-100 text-indigo-700 border border-indigo-200' : 'bg-primary/10 border border-primary/20 text-primary'
                    }`}>Empowering Rural Connectivity</span>
                    <h1 className={`text-6xl md:text-8xl font-extrabold leading-[1.1] tracking-tighter mb-8 ${isModern ? 'text-slate-900' : 'text-slate-100'}`}>
                        Knowledge for <br /><span className={`text-transparent bg-clip-text bg-gradient-to-r ${
                            isModern ? 'from-indigo-600 to-teal-500' : 'from-primary to-accent-teal'
                        }`}>Every Voice.</span>
                    </h1>
                    <p className={`text-lg md:text-xl max-w-2xl mb-10 leading-relaxed ${isModern ? 'text-slate-600' : 'text-slate-400'}`}>
                    ...
                    <div className="flex flex-col sm:flex-row gap-4">
                        <Link to="/login" className={`h-14 px-10 rounded-xl font-bold text-lg hover:scale-105 transition-transform flex items-center justify-center gap-2 ${
                            isModern ? 'bg-indigo-600 text-white shadow-xl shadow-indigo-200' : 'bg-primary text-background-dark'
                        }`}>
                            Try AskBox <span className="material-symbols-outlined">arrow_forward</span>
                        </Link>
                        <button
                            onClick={() => setShowDemoModal(true)}
                            className={`h-14 px-10 rounded-xl font-bold text-lg border transition-colors flex items-center justify-center gap-2 ${
                                isModern 
                                ? 'border-slate-300 text-slate-700 hover:bg-white bg-white/50' 
