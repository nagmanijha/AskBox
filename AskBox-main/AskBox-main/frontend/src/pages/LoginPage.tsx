...
export default function LoginPage() {
    ...
    const handleDemoLogin = async () => {
        ...
        setError('');
        try {
            await login('demo@janvani.ai', 'demo');
            setShowSuccess(true);
            setTimeout(() => navigate('/dashboard'), 1500);
        } catch (err: any) {
            setError('Demo login failed. Please use admin@janvani.ai / askbox to log in.');
        } finally {
            setLoading(false);
        }
    };
            ...
            <div className={`hidden lg:flex lg:w-1/2 flex-col justify-between p-12 relative ${
                ...
                <div>
                    <div className="flex items-center gap-3 mb-2">
                        ...
                        <h2 className={`text-xl font-extrabold tracking-tight ${isModern ? 'text-slate-900' : 'text-slate-100'}`}>JanVani AI</h2>
                    </div>
                ...
                <p className={`text-[10px] ${isModern ? 'text-slate-400' : 'text-slate-600'}`}>© 2024 JanVani AI Intelligence Systems. Team Node — Nagmani Jha</p>
            </div>
                        ...
                        {isLogin && (
                            <div className="pt-2">
                                <button
                                    ...
                                    disabled={loading}
                                    className={`w-full py-3 rounded-xl text-sm font-bold flex items-center justify-center gap-2 transition-colors disabled:opacity-50 border ${
                                        isModern
                                ...
                                <p className="text-[10px] text-slate-500 text-center mt-1.5">No registration required for demo access</p>
                            </div>
                        )}
                    </form>
