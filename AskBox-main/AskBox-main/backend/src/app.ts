...
process.on('SIGTERM', async () => {
    logger.info('SIGTERM received — draining calls and shutting down gracefully');
    await audioPipeline.drain(30000); // Allow 30 seconds for active calls to complete
    server.close(() => process.exit(0));
});

process.on('SIGINT', async () => {
    logger.info('SIGINT received — draining calls and shutting down');
    await audioPipeline.drain(30000);
    server.close(() => process.exit(0));
});
